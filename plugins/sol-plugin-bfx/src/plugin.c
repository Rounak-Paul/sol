...
static const char k_lava_frag[] =
    ...
    /* Base palette: dark rock → theme-warm → theme-hot → white-hot core */
    "    vec3 accent = vec3(pc.r, pc.g, pc.b);\n"
    /* If accent is near neutral (glass theme), fall back to classic lava orange */
    "    float sat = length(accent - vec3(dot(accent, vec3(0.333))));\n"
    "    vec3 hot_col = sat > 0.08 ? accent : vec3(1.0, 0.45, 0.04);\n"
    "    vec3 dark  = hot_col * 0.06;\n"
    "    vec3 warm  = hot_col * 0.38;\n"
    "    vec3 hot   = hot_col;\n"
    "    vec3 white = mix(hot_col, vec3(1.0, 0.95, 0.85), 0.65);\n"
