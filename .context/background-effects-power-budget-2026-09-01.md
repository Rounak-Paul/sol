...
- Causality uses a browser-style quarter-resolution backdrop cache: linear downsample, two fixed 9-tap separable filter passes whose sample spread tracks CSS radius, then linear upsample beneath each backdrop-filter node. This replaces the variable 129-tap-per-axis kernel and gives large CSS radii a fixed, low bandwidth cost.
...
## Frosted glass correction 2026-09-01
