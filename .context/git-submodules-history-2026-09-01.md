...
- Branch creation is a card in the Branches tab. It creates from the current checkout by default; every branch row has a separate Add action that selects that branch as the base without checking it out. The worker uses `git switch -c <new> <base>` only for an explicitly selected base, preserving empty-repository creation from the current HEAD.
